...
func WithTopExecPath(path string) FilterFunc {
	return func(ft *EntryFilter) { ft.TopExecPath = path }
}

// WithTopStream gets the PSEntry from the 'top' stream.
func WithTopStream(str *top.Stream) FilterFunc {
...
func (ft *EntryFilter) applyOpts(opts []FilterFunc) {
	...
	if ft.TopExecPath == "" {
		ft.TopExecPath = top.DefaultExecPath
	}
}
